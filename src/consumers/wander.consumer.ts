...
export class WanderConsumer {
    ...
    async handler(message) {
        ...
        switch (message.properties.KEYS) {
            case 'favorite':
                await this.wanderModel.findOneAndUpdate({ _id: body.wanderId }, { $inc: { favorite: 1 } }).exec();
                break;
            case 'buy':
                await this.wanderModel.findOneAndUpdate({ _id: body.wanderId }, { $inc: { buy: 1 } }).exec();
                break;
            case 'start':
                await this.wanderModel.findOneAndUpdate({ _id: body.wanderId }, { $inc: { start: 1 } }).exec();
                break;
            case 'finish':
